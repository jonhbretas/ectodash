...
import { substituirVoluntario, listarVoluntariosElegiveis } from "./actions";
...
export default function SubstituicaoDialog({
  open,
  onOpenChange,
  escalaId,
  funcao,
  antigoVoluntarioId,
  antigoVoluntarioNome,
  onSubstituido,
}: {
  open: boolean;
  ...
  function handleSubstituir(novoVoluntarioId: number) {
    setMensagem(null);
    startTransition(async () => {
      const result = await substituirVoluntario(
        escalaId,
        antigoVoluntarioId,
        novoVoluntarioId
      );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Substituir voluntário</DialogTitle>
          <DialogDescription>
            Trocar <strong>{antigoVoluntarioNome}</strong> por outro voluntário em{" "}
