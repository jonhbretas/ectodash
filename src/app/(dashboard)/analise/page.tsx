  ...
  return (
    <PageContainer>
      ...
      <div className="grid w-full grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-6">
        ...
        <StatCard label="Concluidas no mes" value={concluidasEsteMes} Icon={CheckCircle2} iconClassName="text-[#2195B9]" />
      </div>
            ...
            {evolucao.map((mes) => (
              <div key={mes.label} className="flex flex-col items-center gap-1.5">
                <div className="flex h-28 w-full items-end justify-center gap-1">
                  <div className="w-3.5 rounded-t-md bg-[#2195B9]" style={{ height: `${Math.max(4, (mes.criadas / maxEvolucao) * 100)}%` }} title={`Criadas: ${mes.criadas}`} role="img" aria-label={`${mes.label}: ${mes.criadas} criadas`} />
        ...
        {rows.map((row, index) => (
          <div key={row.label} className={`flex flex-col gap-1.5 ${index > 0 ? "border-t border-slate-100 pt-3" : ""} ${index < rows.length - 1 ? "pb-3" : ""}`}>
            ...
            <div className="flex h-2.5 w-full overflow-hidden rounded-full bg-slate-100">
              <div className="h-full rounded-full bg-gradient-to-r from-[#2195B9] to-emerald-500" style={{ width: `${(row.total / row.max) * 100}%` }} role="img" aria-label={`${row.label}: ${row.total} demandas, ${row.feitas} concluidas`} />
            </div>
          </div>
        ))}
      </div>
